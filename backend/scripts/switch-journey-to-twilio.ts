...
async function switchJourneyToTwilio() {
  ...
  try {
    ...
    // Update each node's config to set didPoolType to 'Twilio'
    for (const node of nodes) {
      const currentConfig = node.config || {};
      const updatedConfig = {
        ...currentConfig,
        didPoolType: 'Twilio' // Set to Twilio
      };
    ...
    console.log(`\n🎉 Successfully switched ${nodes.length} MAKE_CALL nodes to use Twilio trunk`);
    await dataSource.destroy();
  } catch (error) {
