...
export default function MarketplaceLoginPage() {
  ...
  const [hasLinkedEngine, setHasLinkedEngine] = useState(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await marketplaceApiClient.post('/marketplace/auth/login', {
        email,
        password,
      });
      ...
      // Check if user has linked engine account
      const linkStatus = await UnifiedAuth.checkLinkedEngineAccount();
      if (linkStatus.linked && linkStatus.engineTenantId) {
        setHasLinkedEngine(true);
        // Optionally auto-login to engine
        const engineLoginSuccess = await UnifiedAuth.loginToEngineFromMarketplace(linkStatus.engineTenantId);
        if (engineLoginSuccess) {
          toast.success('Also logged in to Engine');
        }
      }
      ...
      >
        <Card className="glass border-border/50 shadow-xl-modern">
          <CardHeader className="space-y-3 pb-6">
            ...
            <CardTitle className="text-3xl font-bold text-center text-primary">Marketplace Login</CardTitle>
            <CardDescription className="text-center text-base">
              Sign in to access the lead marketplace
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="text-center text-sm pt-2">
                ...
                <Link href="/marketplace/signup" className="font-semibold text-primary hover:text-primary/80 transition-colors">
                  Sign up →
                </Link>
              </div>
              ...
              {hasLinkedEngine && (
                <div className="pt-4 border-t border-border">
                  <p className="text-xs text-center text-muted-foreground mb-2">
                    You also have an Engine account
                  </p>
                  <Link href="/dashboard">
                    <Button 
                      type="button"
                    ...
                    >
                      ...
                      {switchingToEngine ? 'Switching...' : 'Go to Engine'}
                    </Button>
                  </Link>
                </div>
              )}
